...
from finalmodelablation import greedyvig_b_feat
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(np.arange(NBSAMPLES))):
        ...
        for epoch in tqdm(range(NB_EPOCHS)):
            ...
            for i, data in enumerate(train_dataloader, 0):
                ...
                final, out1, out2, out3 = net(inputs)
                loss = criterion(final, labels.type(torch.LongTensor).to(device))
                
                # Additional losses from intermediate outputs
                loss += criterion(out1, labels.type(torch.LongTensor).to(device))
                loss += criterion(out2, labels.type(torch.LongTensor).to(device))
                loss += criterion(out3, labels.type(torch.LongTensor).to(device))
            ...
            if avg_val_loss < best_val_loss:
                ...
                torch.save(net.state_dict(), f'/scratch/das6859/UNetVGNNCAMUS/weights/UnetAblationES_fold{fold+1}_best.pt')
